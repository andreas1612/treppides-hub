...
**Owner:** Andreas Pieri
**Purpose:** Self-hosted internal employee portal replacing SharePoint. Staff land here daily. Pulls live content from BookStack (wiki/KB backend), ClickUp (fees/company dashboard), and Task Manager (project management). LAN-only. Authenticated via Azure AD SSO.
...
| Group Dashboard (unified company list, chart, custom total, cascading filters) | Live |
| Task Manager (dashboard, my tasks, team tasks, create/detail views) | Live |
...
| Authentication --- Azure AD SSO via Task Manager OAuth2 | Live |
...
- **Backend (companies):** FastAPI Python, systemd `companies-api`, port 8003
  - proxied at `/api/companies/*`
- **Backend (tasks):** Spring Boot Java, systemd `taskmanager`, port 8080
  - proxied at `/projects/*` on hub; direct at `tasks.treppides.com`
  - SQL Server on KTDEV:1433, database InternalTools
...
- **Database:** MariaDB in Docker (BookStack), SQLite (valuation + companies), SQL Server (Task Manager)
- **Authentication:** Azure AD SSO --- Spring Boot OAuth2 session, proxied through nginx
- **Deployment:** `git push` from server -> live immediately (frontend); backend services need restart
...
nginx (HTTPS :443, HTTP/2, TLS 1.2+)
  ...
  hub.treppides.com:
    ...
    /api/companies/*   -> localhost:8003 (FastAPI --- company finder)
    /projects/*        -> localhost:8080 (Task Manager, strips /projects prefix)
    /oauth2/*          -> localhost:8080 (OAuth2 authorization)
    /login/oauth2/*    -> localhost:8080 (OAuth2 callback)
    /media/            -> ~/treppides-hub/media/ (static uploaded files)
    http :80           -> redirect -> HTTPS

  tasks.treppides.com:
    /*                 -> localhost:8080 (Task Manager direct access)
...
**Task Manager rebuild (Java changes):**
```bash
...
treppides-hub/
+-- index.html, main.js
+-- login.html                          Hub-branded login page
+-- config.js                           GITIGNORED --- only on server
+-- config.example.js
+-- js/auth.js                          Auth gate (checks /projects/api/me)
...
~/taskmanager/                          Spring Boot Task Manager (separate directory)
...
- Client ID: `dc4895f7-ea14-4387-a368-cbccacee7270` (in Task Manager `application.properties`)
- Client secret: in `application.properties` on server (never commit)
...
- DNS A record: `tasks.treppides.com` -> `192.168.0.221`
- Nginx: new server block for `tasks.treppides.com` proxying to localhost:8080
- Nginx: hub server block updated with `/projects/*`, `/oauth2/*`, `/login/oauth2/*` proxy paths
- Auth flow: `auth.js` checks `/projects/api/me` -> shows hub login page on 401
- `login.html`: hub-branded login page, sends user directly to `/oauth2/authorization/azure`
- Spring Boot redirect-uri auto-generated from request Host header (works for both hub and tasks domains)
- Task Manager accessible both via hub (`/projects`) and directly (`tasks.treppides.com`)
...
10. **Auth proxy paths are critical** --- `/projects/*`, `/oauth2/*`, `/login/oauth2/*` must all proxy to port 8080. Breaking any one breaks the entire auth flow.
