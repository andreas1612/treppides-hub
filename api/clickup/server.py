...
#
# The three AML client forms write straight into the SAME ClickUp lists the
# AML Dashboard reads (new / rejected / disengaged), so they reuse the existing
# CLICKUP_LIST_* vars — no new .env entries needed. A dedicated
# CLICKUP_FORM_*_LIST override is honoured first if you ever want the form to
# target a different list than the dashboard reads.
FORM_LIST_IDS: dict[str, str] = {
    "lead": os.getenv("CLICKUP_FORM_LEAD_LIST", ""),
    "deal": os.getenv("CLICKUP_FORM_DEAL_LIST", ""),
    "new_client":        os.getenv("CLICKUP_FORM_NEW_CLIENT_LIST")   or LIST_IDS.get("new", ""),
    "rejected_client":   os.getenv("CLICKUP_FORM_REJECTED_LIST")     or LIST_IDS.get("rejected", ""),
    "disengaged_client": os.getenv("CLICKUP_FORM_DISENGAGED_LIST")   or LIST_IDS.get("disengaged", ""),
}
for _key, _val in FORM_LIST_IDS.items():
    if not _val:
        logging.warning(f"Form list for '{_key}' not set — the '{_key}' form will be unavailable.")
...
FORMS: dict[str, dict[str, Any]] = {
    ...
    # ------------------------------------------------------------------
    # AML client forms — one per AML Dashboard list. These write into the
    # SAME ClickUp lists the dashboard reads (new / rejected / disengaged),
    # so a user can add a client and see it appear on the dashboard.
    #
    # Field IDs/options below were probed live from the three lists:
    #   new        → 901215683313  "List of new clients"
    #   rejected   → 901217328362  "List of rejected clients"
    #   disengaged → 901217328708  "List of disengaged clients"
    # "Company Name" is the ClickUp task name (native). No status field is
    # shown — new tasks default to the list's first status ("to do").
    # Field order mirrors the live ClickUp forms.
    # ------------------------------------------------------------------

    "new_client": {
        "title": "New Client",
        "list_env": "new_client",
        "subtitle": "Add a new client into the New Clients list.",
        "status_info": [],
        "sections": [
            {"heading": None, "fields": [
                {"key": "company_name", "label": "Company Name", "native": "name",
                 "type": "text", "required": True, "placeholder": "Enter text"},
                {"key": "engagement_leader", "label": "Engagement Leader", "cf_id": "32e8fccf-7dc7-4797-92b9-fb488e078659",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "a577e2bd-09df-4243-8de2-57d676b0debe", "name": "SY"},
                    {"id": "f7aa343c-007b-4e23-8c3d-02d1d8438f0b", "name": "AD"},
                    {"id": "25651818-551d-409a-8329-9a98a32f3c57", "name": "MS"},
                    {"id": "67f9130e-0c67-451a-bcbb-be0dce5b007e", "name": "CA"},
                    {"id": "b33e852a-5975-49ea-ab2a-78f4626ac2ce", "name": "MX"},
                    {"id": "0538de3a-f91d-47bc-a07e-8b0a1ac013a9", "name": "GS"},
                    {"id": "b05348ef-3357-49e2-b1fb-7493a4a7e4f6", "name": "GP"},
                    {"id": "3a26dba1-3e1d-4a3c-9ab8-6fddf066e169", "name": "ME"},
                    {"id": "b6fc73f0-b799-4f94-a5ba-cd9503cc6f95", "name": "MP"},
                    {"id": "071209f9-710a-4fd4-9df7-aade6c5fee4f", "name": "GNP"},
                    {"id": "be3c5267-e94a-41d4-a9e6-7987177f817e", "name": "NK"},
                    {"id": "b9ff06ba-f745-4ab9-85ad-26a27f1092f9", "name": "AV"},
                    {"id": "9b7044c4-3b4f-4af7-9c1f-793e9fdc3234", "name": "KH"},
                    {"id": "780b9783-1660-4743-b1b5-9ef0809d5a89", "name": "AP"},
                    {"id": "8655947f-51ae-40f3-b557-281a83d9c36d", "name": "PM"},
                 ]},
                {"key": "month", "label": "Month", "cf_id": "f14f6e29-9e8e-423f-acfe-400b624f7a16",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "f265067b-4858-4a19-af90-10bccd7808e1", "name": "January"},
                    {"id": "7dba3cd3-f947-4a3b-a154-4260568b0c37", "name": "February"},
                    {"id": "79e6d78c-4cc8-4ca8-af27-8f4c430b7bba", "name": "March"},
                    {"id": "7ee02cb9-f9fe-4f58-b218-6f3f3bf6a926", "name": "April"},
                    {"id": "f76b0e76-031d-4107-abea-e161e36d8221", "name": "May"},
                    {"id": "deaf9b6b-360a-4cf2-b43a-e95745715cc2", "name": "June"},
                    {"id": "f06c42c4-ad30-441c-bcfa-321a6b2921f9", "name": "July"},
                    {"id": "3b2a3438-ad6e-428d-bfd2-dab69d0befef", "name": "August"},
                    {"id": "13c673d0-3832-4463-bac8-08b215d519f1", "name": "September"},
                    {"id": "e67b729d-59d2-4149-9123-40579a40e30e", "name": "October"},
                    {"id": "66821697-e670-403e-8925-9f7b9df9102d", "name": "November"},
                    {"id": "e7d3519f-526a-426d-983f-9222347f99a7", "name": "December"},
                 ]},
                {"key": "year", "label": "Year", "cf_id": "e4358010-37cb-412d-81b0-7c3727478dde",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "b7d30baa-f3d2-4b7a-ad4c-48ec418363cb", "name": "2025"},
                    {"id": "feb0640b-280a-4508-aff5-8e682207732f", "name": "2026"},
                    {"id": "e04aa218-8640-4745-bf13-97d18b269318", "name": "2027"},
                    {"id": "370e5154-4d88-48c1-8235-fea1c6f0319d", "name": "2028"},
                    {"id": "ec37fab2-a491-43d7-af0a-9cd75d1de924", "name": "2029"},
                    {"id": "75e725c4-48c7-44f7-a4d6-3974b66fb568", "name": "2030"},
                 ]},
                {"key": "ubo", "label": "UBO", "cf_id": "adedaac5-a2f3-48c3-b931-4e7e21eb352b",
                 "type": "text", "required": True, "placeholder": "Enter text"},
                {"key": "group_name", "label": "Group Name", "cf_id": "ae29f0d4-2a22-46d5-bfbb-baec6122ac7a",
                 "type": "text", "required": True, "placeholder": "Enter text"},
                {"key": "newgroup", "label": "NewGroup?", "cf_id": "ffb61a0c-1f08-4204-9e54-4584d2583b72",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "12e536fa-6406-418f-8b74-1f3f65a43bd0", "name": "Yes"},
                    {"id": "df54a602-f298-46b9-be08-bc56d5a69922", "name": "No"},
                 ]},
                {"key": "client_status", "label": "Client Status", "cf_id": "9104cfd4-7c5b-4883-bfb0-a15e3a3c13c5",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "ec04fffc-a6fb-4084-bce3-ab9ed7d1310c", "name": "Existing"},
                    {"id": "521c50af-85fc-40eb-b4d3-5d5fa5bebd9f", "name": "New"},
                 ]},
                {"key": "categorization", "label": "Categorization", "cf_id": "5567094a-eeca-46b9-9e03-a3f742ef79dc",
                 "type": "textarea", "required": True, "placeholder": "Enter text"},
                {"key": "managing_company", "label": "Managing Company", "cf_id": "695a9b1b-0673-4710-8e0a-6e18247adc9d",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "3d81ee10-d85c-48d7-a9a2-8797710d1ff6", "name": "Finanz-Audit Limited"},
                    {"id": "1051044f-9abe-4fa8-aebc-41e93fba253d", "name": "K. Treppides & Co Ltd"},
                    {"id": "878d8174-5b61-4dee-8a49-30a0eb0042a0", "name": "K. Treppides & Co (Uk) Ltd"},
                    {"id": "9bc7ec3c-ebbe-4e34-8599-8030337afa08", "name": "Treppides Fund Services Ltd"},
                    {"id": "e8508df6-607d-44a8-88c0-63106428cae6", "name": "Treppides Advisers Ltd"},
                    {"id": "e7464751-3b5b-49de-a797-24d02dc4c34d", "name": "Nomuscapital Investments Ltd"},
                 ]},
                {"key": "introducer", "label": "Introducer", "cf_id": "79977bae-b01a-43b1-a0f1-d95814df5377",
                 "type": "text", "required": True, "placeholder": "Enter text"},
                {"key": "cross_sell_to", "label": "Cross sell (Yes or No)", "cf_id": "23dae3a4-7f3f-41d5-a4b8-6b7ad810faee",
                 "type": "text", "required": True, "placeholder": "Enter text"},
                {"key": "departement", "label": "Department", "cf_id": "96806d3b-9f2d-4dd8-bc13-d09123e3a6f9",
                 "type": "labels", "required": True, "options": [
                    {"id": "86027df6-19c3-4da3-98d7-9c2b9b3c646a", "name": "Audit"},
                    {"id": "f43de04a-d9c0-4cab-bb3c-674bdf4f987f", "name": "FCR"},
                    {"id": "3b638a9a-f5e2-4b8a-bc35-d73b86bc952c", "name": "FRA"},
                ...
                {"key": "service", "label": "Services", "cf_id": "1c4cf919-3756-44a4-b864-b6db9302efcd",
                 "type": "labels", "required": True, "options": [
                    {"id": "d1bc883c-5a16-4e8a-be0e-d458886cbcca", "name": "Accounting"},
                    {"id": "f1834867-80b0-4114-9fa7-3bf018111759", "name": "AML Audit & Gap Analysis"},
                    {"id": "9c8e79eb-b716-43ec-80bf-8548c8d9b62a", "name": "Audit"},
                ...
                {"key": "meeting_with_natural_person", "label": "Meeting with Natural person",
                 "cf_id": "b750ee6e-4a93-4016-8a21-3adec97d6f90", "type": "date", "required": True},
                {"key": "fees", "label": "FEES", "cf_id": "1b235c14-b8a9-4d1e-b52b-7dc9dea9bceb",
                 "type": "currency", "required": True, "placeholder": "Enter currency"},
                {"key": "supervisor___manager", "label": "SUPERVISOR / MANAGER", "cf_id": "a984eb39-8416-4594-ae65-da69302fa7da",
                 "type": "text", "required": True, "placeholder": "Enter text"},
                {"key": "new_clients_details", "label": "New Clients Contact Details",
                 "cf_id": "cbd0ce87-c693-4405-82c7-a661d6035b7b", "type": "textarea", "required": True,
                 "placeholder": "Emails, phones, etc"},
            ]},
        ],
    },
    ...
    "rejected_client": {
        ...
        "list_env": "rejected_client",
        "subtitle": "Add a client into the Rejected Clients list.",
        "status_info": [],
        "sections": [
            {"heading": None, "fields": [
                {"key": "company_name", "label": "Company Name", "native": "name",
                ...
                {"key": "rejection_reason", "label": "Rejection Reason", "cf_id": "ac97bcc4-88c7-4332-a38b-708bc384919d",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "c501d25f-30f6-43ad-8c71-039ab4281554", "name": "Regulatory Non-Compliance"},
                    {"id": "5b04d1ea-aa55-478f-b2d5-789e029d83ec", "name": "Incomplete Documentation"},
                    {"id": "e57d0449-38c1-4b6a-a1e7-0b73be175148", "name": "Insufficient Financial Information"},
                    {"id": "490791bf-d451-4914-b4ba-458e3b09fb78", "name": "Policy Violation"},
                    {"id": "ad2c6792-772a-4680-81a4-ec71c2ac8fbd", "name": "Risk Assessment Failure"},
                 ]},
                {"key": "month", "label": "Month", "cf_id": "f14f6e29-9e8e-423f-acfe-400b624f7a16",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "f265067b-4858-4a19-af90-10bccd7808e1", "name": "January"},
                ...
                {"key": "group_name", "label": "Group Name", "cf_id": "ae29f0d4-2a22-46d5-bfbb-baec6122ac7a",
                 "type": "text", "required": True, "placeholder": "Enter text"},
                {"key": "rejected_details", "label": "Rejected Details", "cf_id": "2e1f8fca-ae1c-410b-96a7-d39dd3e87342",
                 "type": "textarea", "required": True, "placeholder": "Enter text"},
            ]},
        ],
    },
    ...
    "disengaged_client": {
        "title": "Disengaged Client",
        "list_env": "disengaged_client",
        "subtitle": "Add a client into the Disengaged Clients list.",
        "status_info": [],
        "sections": [
            {"heading": None, "fields": [
                {"key": "company_name", "label": "Company Name", "native": "name",
                ...
                {"key": "disengaged_reason", "label": "Disengaged reason", "cf_id": "c24e3d90-105d-4ac1-bc3a-b33bf2126354",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "3914e681-f93b-4835-ae40-fc9c9a565f93", "name": "Not accepted increase of fees"},
                    {"id": "0ecc0afd-a97e-4254-a35e-eee2260edba0", "name": "KYC matters"},
                    {"id": "ee13063c-22e7-4915-bb74-ab3296db21d9", "name": "Long outstanding - Legal proceedings"},
                    {"id": "4509960f-cb12-44f1-a7b1-3780c52dc8fe", "name": "Client decision"},
                 ]},
                {"key": "month", "label": "Month", "cf_id": "f14f6e29-9e8e-423f-acfe-400b624f7a16",
                 "type": "drop_down", "required": True, "options": [
                    {"id": "f265067b-4858-4a19-af90-10bccd7808e1", "name": "January"},
                ...
                {"key": "group_name", "label": "Group Name", "cf_id": "ae29f0d4-2a22-46d5-bfbb-baec6122ac7a",
                 "type": "text", "required": True, "placeholder": "Enter text"},
                {"key": "disengagement_details", "label": "Disengagement details", "cf_id": "622a0e8d-86bf-4ab3-974a-d82a7561b5fe",
                 "type": "textarea", "required": True, "placeholder": "Enter text"},
            ]},
        ],
    },
