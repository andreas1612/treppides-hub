...
const STEPS = [
  ...
  {
    title: "Assets vs Financing",
    body: "This chart shows what the balance sheet is made of — total assets on the left, how they're funded (liabilities + equity) on the right. When everything's positive the two bars match height, confirming it balances. A negative section (e.g. accumulated-loss equity) shows as a red reducing band, with a note flagging it — a quick visual read of the company's financial structure.",
    anchor: "#tbr-bs-composition",
    tab: "bs",
    requiresData: true,
    placement: "top",
  },
  {
    ...
    body: "A gauge for the current ratio (current assets ÷ current liabilities), with the firm's health bands — Good, Caution, Bad — and a marker for where this period sits. Load a comparative and a dashed tick shows the prior year too.",
    anchor: "#tbr-bs-gauge",
    tab: "bs",
    requiresData: true,
    placement: "top",
  ...
  },
  {
    ...
    body: "This cascade shows how revenue becomes net profit — grey bars are the running subtotals (Revenue → Gross → Operating → Net), red bars are the deductions between them, and the dotted line carries each subtotal to the next step. If the business is loss-making, the bars step below zero into the negative subtotals; steps that are zero are left out to keep it clean.",
    anchor: "#tbr-pnl-waterfall",
    tab: "pnl",
    requiresData: true,
    placement: "top",
  },
  ...
  },
  {
    ...
    body: "Set the base file name here before exporting — it defaults to the uploaded file's name. Every download uses it, with a suffix added automatically (…_financial_statements.xlsx, …_balance_sheet.pdf, …_profit_and_loss.pdf).",
    anchor: "#tbr-export-name",
    tab: "pnl",
    requiresData: true,
    placement: "top",
  },
  ...
  {
    title: "Export to PDF",
    body: "Open this menu to download a PDF: both statements together, the Balance Sheet only, or the Profit & Loss only. Each option includes that statement's table, its charts and its ratios — a ready-to-share report.",
    anchor: "#tbr-pdf-menu",
    tab: "pnl",
    requiresData: true,
    placement: "top",
